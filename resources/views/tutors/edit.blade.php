                ...
                <x-card title="Data Diri Tutor">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            <x-input label="Nama Lengkap" name="name" :value="old('name', $tutor->name)" placeholder="Nama tutor"
                                :error="$errors->first('name')" />
                        </div>
                        <x-input label="No. Telepon" name="telp" :value="old('telp', $tutor->telp)" placeholder="08xxxxxxxxxx"
                            :error="$errors->first('telp')" />
                        <x-input label="Tanggal Lahir" name="dob" type="date" :value="old('dob', $tutor->dob?->format('Y-m-d'))" :error="$errors->first('dob')" />
                        <div class="sm:col-span-2">
                            <x-input label="Domisili" name="domicille" :value="old('domicille', $tutor->domicille)" placeholder="Kota domisili"
            ...
            <div x-data="{ search: '' }">
                <x-card title="Kelas & Gaji per Sesi">
                    ...
                    <div class="space-y-5 max-h-[380px] overflow-y-auto pr-1">
                                ...
                                <div class="space-y-3">
                                    @foreach ($group as $class)
                                        @php
                                            ...
                                            $rawAmount = old(
                                                'amounts.' . $class->id,
                                                $assignedAmounts->get($class->id, 0),
                                            );
                                            $currentAmount = $rawAmount > 0 ? $rawAmount : $defaultRate;
                                            $currentExtraFee = old(
                                                'extra_fees.' . $class->id,
                                                $assignedExtraFees->get($class->id, 0),
                                            );
                                            // Anak private mana yang dipegang: pakai old → assigned → default semua
                                            $pupilChecked = function ($pupilId) use ($isChecked, $assignedPupilIds) {
                                                if (old('pupil_ids') !== null) {
                                                    return in_array($pupilId, old('pupil_ids', []));
                                                }
                                                return $isChecked ? $assignedPupilIds->contains($pupilId) : true;
                                            };
                                        @endphp
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="space-y-2">
                                                            @foreach ($class->pupils as $p)
                                                                @php
                                                                    $pExtra = old('pupil_extra.' . $p->id, $assignedPupilExtras->get($p->id, 0));
                                                                @endphp
                                                                <div x-data="{ on: {{ $pupilChecked($p->id) ? 'true' : 'false' }} }"
                                                                    ...
                                                                    <div class="relative w-24 shrink-0" x-show="on" x-transition x-cloak>
                                                                        ...
                                                                        <input type="number" name="pupil_extra[{{ $p->id }}]"
                                                                            value="{{ $pExtra }}" min="0" step="500" placeholder="0"
                                                ...
                                                <div>
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <input type="number" name="amounts[{{ $class->id }}]"
                                                            value="{{ $currentAmount }}" min="0" step="500"
                                                            placeholder="0"
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="relative">
                                                            ...
                                                            <input type="number" name="extra_fees[{{ $class->id }}]"
                                                                value="{{ $currentExtraFee }}" min="0" step="500"
                                                                placeholder="0"
                                                                class="w-full rounded-lg border border-gray-300 pl-9 pr-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">
                                                        </div>
                                                    </div>
                        ...
                        @empty
                            <p class="text-sm text-gray-400 text-center py-4">Belum ada kelas.</p>
                        @endforelse
