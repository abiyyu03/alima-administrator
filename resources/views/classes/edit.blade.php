...
@section('content')
<form method="POST" action="{{ route('classes.update', $class) }}" x-data="classForm()">
    @csrf @method('PUT')
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <x-card title="Data Kelas">
            <div class="space-y-4">
                <x-input label="Nama Kelas" name="name" :value="old('name', $class->name)"
                ...
                <x-select label="Jenis Kursus" name="course_type_id" :error="$errors->first('course_type_id')"
                    ...
                    @foreach($courseTypes as $type)
                        <option value="{{ $type->id }}" data-name="{{ strtolower($type->name) }}"
                            @selected(old('course_type_id', $class->course_type_id) == $type->id)>
                            {{ $type->name }}
                        </option>
                    @endforeach
                </x-select>
